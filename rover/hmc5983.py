...
ID_REGISTER_B = 0x0B
ID_REGISTER_C = 0x0C
TEMP_MSB_REGISTER = 0x31
TEMP_LSB_REGISTER = 0x32
...
class Magnetometer(object):
    ...
    def get_heading(self, direction: str = 'xy'):
        (x, y, z) = self.get_data()
        if direction == 'yz':
            x_prime = y
            y_prime = z
        ...
        heading = math.atan2(y_prime, x_prime) + self.__declination
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print('X-Y Heading: {:d}\N{DEGREE SIGN} {:d}\''.format(degrees,
                                                               minutes))
        heading = magnetometer.get_heading(direction='yz')
        degrees, minutes = convert_radians_to_degrees_minutes(heading)
        print('Y-Z Heading: {:d}\N{DEGREE SIGN} {:d}\''.format(degrees,
                                                               minutes))
        heading = magnetometer.get_heading(direction='zx')
        degrees, minutes = convert_radians_to_degrees_minutes(heading)
        print('Z-X Heading: {:d}\N{DEGREE SIGN} {:d}\''.format(degrees,
                                                               minutes))
        time.sleep(0.5)
